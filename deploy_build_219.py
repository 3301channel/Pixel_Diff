"""部署到 192.168.2.19（CentOS 7 x86_64，root + venv）并 PyInstaller 打包。

与 deploy_build_221.py 的区别：
- 目标机是 CentOS 7（glibc 2.17），依赖需 manylinux2014 版本
- Python 环境用 /opt/pd_build_venv（Python 3.11.9 + venv）
- 依赖：numpy 1.26.4 / opencv-contrib-python-headless 4.11.0.86 / pymupdf 1.24.14

用法（凭据走环境变量）：
    PD_BUILD_HOST=192.168.2.19 PD_BUILD_USER=root PD_BUILD_PWD='Hl@2026' \
        python deploy_build_219.py [sync|build|all]
"""
...
HOST = os.environ.get("PD_BUILD_HOST", "192.168.2.19")
...
PYINSTALLER = os.environ.get("PD_PYINSTALLER", "/opt/pd_build_venv/bin/pyinstaller")
...
def do_build(remote_proj: str | None = None) -> int:
    ...
    c = connect()
    build_cmd = f"cd {remote_proj} && {PYINSTALLER} --clean -y build_linux_kylin.spec"
    print(f"[build] 开始打包（CentOS7 4 核，预计 3~6 分钟）...")
