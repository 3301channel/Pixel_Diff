...
def remove_colored_marks_bgr(
    image_bgr: np.ndarray,
    config: PixelDiffConfig,
    *,
    protected_zero_mask: np.ndarray | None = None,
) -> np.ndarray:
    ...
    if protected_zero_mask is not None:
        if protected_zero_mask.shape != mask.shape:
            raise ValueError("protected_zero_mask must match the image height and width")
        mask[protected_zero_mask == 0] = 0
