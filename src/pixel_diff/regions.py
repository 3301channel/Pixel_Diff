...
def filter_locally_similar_regions(
    ...
    if (
        ...
        return _renumber_regions(regions)
...
def _best_local_iou(
    ...
    for dy in range(-search_radius, search_radius + 1):
        for dx in range(-search_radius, search_radius + 1):
            ...
            # 交集：两幅图都有文字的像素数
            intersection = np.logical_and(scan_crop, shifted_template).sum()
            # 并集：至少一幅图有文字的像素数
            union = np.logical_or(scan_crop, shifted_template).sum()
...
def _renumber_regions(regions: list[DifferenceRegion]) -> list[DifferenceRegion]:
    """重新编号：对过滤后的保留区域，从 1 开始连续编号。

    过滤操作可能删除部分区域，导致编号不连续。
    此函数在过滤后重新分配连续的 id。
    """
    return [
        DifferenceRegion(
