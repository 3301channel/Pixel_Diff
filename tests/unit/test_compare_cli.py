...
from pixel_diff import ConfigurationError, InputError
...
def test_load_config_rejects_missing_explicit_path(tmp_path: Path) -> None:
    ...
    with pytest.raises(ConfigurationError, match="configuration file does not exist"):
        compare_cli._load_config(str(tmp_path / "missing.yaml"))
...
def test_prepare_output_paths_creates_report_layout(tmp_path: Path) -> None:
    ...
    assert output_paths.candidate == expected_report_dir / "page_0001_candidate.jpg"
    assert output_paths.ghost == expected_report_dir / "page_0001_heatmap.png"
    assert output_paths.patch == expected_report_dir / "patches" / "page_0001"
...
def test_page_timing_line_and_report_summary_include_elapsed_times(tmp_path: Path) -> None:
    compare_cli = _load_compare_cli()
    result = _FakeResult(page=1, differences=[object()])
    result.metrics = {
        "elapsed_ms": 12345,
        "timing_render_ms": 1200,
        "timing_global_alignment_ms": 3400,
        "timing_local_warp_ms": 2500,
        "timing_filter_ms": 1800,
        "timing_output_ms": 500,
    }
    line = compare_cli._format_page_timing(result, total_pages=5)

    assert "page 2/5" in line
    assert "time=12.35s" in line
    assert "render=1.20s" in line
    assert "warp=2.50s" in line
    ...
    payload = {
        "pages": [{"page": 1, "elapsed_ms": 12345}],
        "total_regions": 1,
        "wall_elapsed_ms": 7000,
    }
    summary = compare_cli._format_report_summary(payload, output_paths)

    assert "algorithm_time=12.35s" in summary
    assert "wall_time=7.00s" in summary
...
def test_report_page_output_paths_use_one_based_page_names(tmp_path: Path) -> None:
    ...
    assert page_paths["template_output_path"].endswith("page_0002_original.jpg")
    assert page_paths["candidate_output_path"].endswith("page_0002_candidate.jpg")
    assert page_paths["ghost_output_path"].endswith("page_0002_heatmap.png")
    assert page_paths["patch_output_dir"].endswith("patches\\page_0002")
...
def test_effective_report_workers_allows_three_pages_in_parallel() -> None:
    compare_cli = _load_compare_cli()

    assert compare_cli._effective_report_workers(page_count=5, configured_workers=3) == 3
