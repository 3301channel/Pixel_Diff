...
def test_vectorized_local_iou_matches_legacy_search_at_edges() -> None:
    from pixel_diff.regions import _best_local_iou, _best_local_iou_vectorized

    generator = np.random.default_rng(20260721)
    scan = (generator.random((80, 95)) > 0.82).astype(np.uint8)
    template = (generator.random((80, 95)) > 0.82).astype(np.uint8)
    regions = [
        DifferenceRegion(id=1, x=0, y=0, width=19, height=17, area=200.0),
        DifferenceRegion(id=2, x=32, y=26, width=27, height=21, area=400.0),
        DifferenceRegion(id=3, x=78, y=68, width=17, height=12, area=150.0),
    ]

    for region in regions:
        legacy = _best_local_iou(region, scan, template, padding=7, search_radius=5)
        vectorized = _best_local_iou_vectorized(
            region,
            scan,
            template,
            padding=7,
...
def test_filter_locally_similar_regions_removes_wide_row_with_larger_shift() -> None:
    template = np.full((140, 760), 255, dtype=np.uint8)
    scan = np.full((140, 760), 255, dtype=np.uint8)
    cv2.putText(
        template,
        "FULL LINE RESIDUAL SHOULD ALIGN WITH BIGGER SEARCH",
        (30, 70),
    ...
    cv2.rectangle(scan, (40, 105), (78, 128), 0, -1)
    regions = [
        DifferenceRegion(id=1, x=25, y=42, width=700, height=50, area=12000.0),
        DifferenceRegion(id=2, x=40, y=105, width=39, height=24, area=850.0),
    ]
    config = PixelDiffConfig(
        local_similarity_iou_threshold=0.90,
        local_similarity_search_radius=4,
        wide_text_residual_min_iou=0.90,
        wide_row_residual_min_width=500,
        wide_row_residual_max_height=90,
        wide_row_residual_search_radius=12,
        wide_row_residual_min_iou=0.45,
    )

    filtered = filter_locally_similar_regions(regions, scan, template, config)

    assert [(region.id, region.x, region.y) for region in filtered] == [(1, 40, 105)]
